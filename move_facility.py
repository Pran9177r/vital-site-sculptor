...
# 1. Create app/facility/page.tsx
os.makedirs("app/facility", exist_ok=True)
with open("app/facility/page.tsx", "w") as f:
    f.write('''"use client";
...
page_content = re.sub(r'import \{ FacilityGallery \} from "@/components/FacilityGallery";\n', '', page_content)
page_content = re.sub(r'import \{ Map, MapMarker, MarkerContent, MarkerTooltip \} from "@/components/ui/mapcn-marker-tooltip";\n', '', page_content)
page_content = re.sub(r'const FACILITY_IMAGES = \[.*?\];\n\n', '', page_content, flags=re.DOTALL)
page_content = re.sub(r'\s*\{\/\* Our Facility \*\/\}.*?(?=\{\/\* Testimonials \*\/\})', '\n\n      ', page_content, flags=re.DOTALL)
...
about_content = re.sub(r'import \{ FacilityGallery \} from "@/components/FacilityGallery";\n', '', about_content)
about_content = re.sub(r'import \{ Map, MapMarker, MarkerContent, MarkerTooltip \} from "@/components/ui/mapcn-marker-tooltip";\n', '', about_content)
about_content = re.sub(r'const FACILITY_IMAGES = \[.*?\];\n\n', '', about_content, flags=re.DOTALL)
about_content = re.sub(r'\s*\{\/\* Our Facility \*\/\}.*?(?=\s*<\/div>\s*<\/div>\s*;\s*\}\s*$)', '\n\n', about_content, flags=re.DOTALL)
